...
class BertDataset(object):
    ...
    def __init__(self, bert_path, max_length: int = 512):
        ...
        self.tokenizer.mask_token_id = self.tokenizer.token_to_id("[MASK]")
        self.tokenizer.cls_token_id = self.tokenizer.token_to_id("[CLS]")
        self.tokenizer.sep_token_id = self.tokenizer.token_to_id("[SEP]")
        self.tokenizer.unk_token_id = self.tokenizer.token_to_id("[UNK]")
    ...
    def encode(self, sentence,add_special_tokens=True):
        ...
        tokenizer_output = self.tokenizer.encode(sentence,add_special_tokens=add_special_tokens)
        ...
        pinyin_ids = torch.LongTensor(pinyin_tokens).view(-1).tolist()
